...
- 未命中时走 RAG 生成；生成后若满足可缓存条件，将「问题 / 答案 / 来源」写入缓存，供后续相似问题复用。
...
- 启动前置检查：导入流程强依赖向量库与 Embedding 服务，任一不可用时报错并阻止启动，不做降级导入，避免"文件哈希已登记但向量化被跳过、后续增量同步又因哈希未变而不再补偿"的状态。
...
1. **启动装载缓存**：系统启动时，从 SQLite 读取所有 `status = active` 的问答缓存条目，仅将「问题 + 问题向量」装入内存，构建内存匹配索引；`answer` 与 `sources` 不常驻内存。运行期新增缓存后同步追加该索引。
...
3. **重排序与去重**：调用 `BAAI/bge-reranker-v2-m3` 对候选片段进行二次交叉注意力打分，截取 `top-n`（如 Top 5）最贴切的片段并按 `source_id` + `chunk_index` 剔除冗余。
4. **上下文组装与生成**：将原始问题（保持用户原意）与重排序后的片段组装进 Prompt 模板，调用 LLM 生成回答。
5. **结构化返回**：返回最终回答及引用来源列表，包含 `source_type`（`post` / `document`）、`source_id`、`title` 和片段内容；所有引用来源均仅展示片段、不提供跳转。
6. **写入问答缓存**：当本次回答满足可缓存条件时，将「用户原始问题、生成的答案、引用来源」以及该问题的稠密向量写入 SQLite 缓存表（`expires_at = now + cache_ttl`），并把「问题 + 问题向量」追加进内存匹配索引；缓存生效后，后续相似问题可由阶段一命中。

> **可缓存条件**：仅当 `sources` 非空、引用来源的重排得分达到设定的最低置信度，且 `answer` 不是「未检索到相关内容 / LLM 未配置」等兜底文案时，才写入缓存，避免把低质量的回答固化进缓存。
