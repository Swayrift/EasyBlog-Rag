...
- 使用项目内的 Markdown 结构化切块器按规则切块。
- 标题与其后续内容保持关联，代码块、表格、列表、引用等结构不会在普通边界被破坏；超长结构才会按行拆分。
- 不依赖第三方语义切块库，避免其对代码块和重复内容处理不一致。
- `chunk_min_size` 为最小目标长度，`chunk_size` 为软上限，`chunk_hard_size` 为绝对硬上限。
- 切块阶段不调用 Embedding；Embedding 只在切块完成后为最终 Chunk 生成向量。
